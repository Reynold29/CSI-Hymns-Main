...
class GitHubSyncService {
  ...
  Future<bool> pushToGitHub(List<ChristmasCarol> carols) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        debugPrint('GitHubSyncService: Failed to push - ${response.statusCode}: ${response.body}');
        return false;
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> _getFileContent(String repo, String filePath, String token) async {
    try {
      final url = 'https://api.github.com/repos/$repo/contents/$filePath';
      final headers = {
        'Authorization': 'token $token',
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('GitHubSyncService: Failed to get file - ${response.statusCode}');
        return null;
      }
    } catch (e) {
  ...
  Future<List<ChristmasCarol>?> pullFromGitHub() async {
    try {
      ...
      final decodedContent = utf8.decode(base64Decode(content.replaceAll('\n', '')));
